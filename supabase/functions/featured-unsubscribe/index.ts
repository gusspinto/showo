...
/* ══════════════════════════════════════════════════════════════════════════
   UNSUBSCRIBE DO AVISO "PROJETO EM DESTAQUE" — mesmo padrão do
   checkin-unsubscribe e onboarding-unsubscribe (HMAC + one-click RFC 8058),
   grava numa coluna própria (featured_notification_opted_out), lista
...
Deno.serve(async (req) => {
  ...
  await supabase.from('profiles').update({ featured_notification_opted_out: true }).eq('id', userId)

  if (req.method === 'POST') return new Response(null, { status: 200 })

  return new Response(
    page('Cancelado', 'Já não vais receber avisos de projetos em destaque. O resto da tua conta continua na mesma.'),
    { headers: { 'Content-Type': 'text/html; charset=utf-8' } },
  )
})
