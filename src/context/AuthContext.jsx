...
export function AuthProvider({ children }) {
  ...
  const fetchProfile = useCallback(async (uid) => {
    ...
    const [profileRes, userRes] = await Promise.all([
      supabase.from('profiles').select('id, username, full_name, bio, is_admin, banned_at, role, avatar_url, available_for_work, linkedin_url, skills').eq('id', uid).single(),
    ...
    if (!data) {
      const { data: created } = await supabase
        ...
        .select('id, username, full_name, bio, is_admin, banned_at, role, avatar_url, available_for_work, linkedin_url, skills')
        .single()
      data = created
    }
